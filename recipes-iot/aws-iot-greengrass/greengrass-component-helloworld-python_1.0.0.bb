...
do_install:append() {
    if [ "${GREENGRASS_VARIANT}" = "lite" ]; then
        install -m 0755 ${WORKDIR}/hello_world.py ${D}${GGL_ARTIFACTS_DIR}/${COMPONENT_NAME}/${COMPONENT_VERSION}/
    fi
}
...
do_install_ptest() {
    ...
    # Copy the component's recipe and Python script for testing
    install -m 0644 ${WORKDIR}/component-recipe.yaml ${D}${PTEST_PATH}/test-data/
    install -m 0644 ${WORKDIR}/hello_world.py ${D}${PTEST_PATH}/test-data/
