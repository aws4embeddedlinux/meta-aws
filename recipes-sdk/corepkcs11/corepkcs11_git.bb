...
do_configure:prepend() {
    ...
    cp ${UNPACKDIR}/corepkcs11_mbedtls_config.h ${S}/source/dependency/3rdparty/mbedtls/include/mbedtls/mbedtls_config.h
    cp ${UNPACKDIR}/CMakeLists.txt_mbedtls ${S}/source/dependency/3rdparty/mbedtls/CMakeLists.txt
    cp ${UNPACKDIR}/core_pkcs11_config.h ${S}/source/include/
    cp ${UNPACKDIR}/logging_levels.h ${S}/source/include/
    cp ${UNPACKDIR}/logging_stack.h ${S}/source/include/
}

do_install:append() {
    install -d ${D}${datadir}/cmake/Modules
    install -m 0644 ${UNPACKDIR}/Findcore_pkcs.cmake ${D}${datadir}/cmake/Modules/
    cp ${S}/source/dependency/3rdparty/pkcs11/published/2-40-errata-1/* ${D}/${includedir}/core_pkcs/
}

FILES:${PN} += "${libdir}/libcore_pkcs.so.*"
FILES:${PN}-dev += "\
    ${libdir}/libcore_pkcs.so \
